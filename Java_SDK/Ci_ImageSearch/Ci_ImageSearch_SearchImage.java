...
public class Ci_ImageSearch_SearchImage {
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        // 4 bucket名需包含appid
        String Bucket = "";
        // 5 存储在存储桶内图片的对象键，图片大小不能超过2M，必须以/开头
        String key = "";
        ...
        long limit = 10;
        // 9 针对入库时提交的 Tags 信息进行条件过滤。支持>、>=、<、<=、=、!=，多个条件之间支持 AND 和 OR 进行连接; 举例：key!="value"
        String filter = URLEncoder.encode("","utf-8");
        // 10 生成此次请求用的HOST
        ...
        Request request = new Request.Builder()
                .url("https://" + host + key + "?ci-process=ImageSearch&action=SearchImage&MatchThreshold=" + matchthreshold + "&Offset=" + offset + "&Limit=" + limit + "&Filter=" + filter )
                .method("GET",null)
        ...
        try {
            ...
            System.out.println("<" + response.code() + ">" + "\r\n" + response.body().string() + "\r\n" + response.headers());  //如果返回400，请检查参数部分是否传入有误，或者该图片是否有添加到图库中
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
