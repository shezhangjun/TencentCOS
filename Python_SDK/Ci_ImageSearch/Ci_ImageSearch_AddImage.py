...
entityid = ''      #图片添加至图库中定义的物品 ID
customcontent = '' #图片添加至图库中自定义的内容，最多支持4096个字符，查询时原样带回
tags = '{"key1": "value1", "key2": "value2"}'  #图片添加至图库中，图片自定义标签，最多不超过10个，json 字符串，格式为 key:value 对
host = bucket + '.cos.' + region + '.myqcloud.com'                 #生成此次请求用的HOST
url = scheme + '://' + host + key + '?ci-process=ImageSearch&action=AddImage'
jsonbody = {"Request": [{"EntityId": entityid, "CustomContent": customcontent, "Tags": [tags]}]}
...
if response.status_code == 200:
    print(f'\033[0;32mSuccess Add Image!\033[0m')
